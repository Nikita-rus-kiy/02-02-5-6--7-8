...
public class Main5 {
        ...
        for (RecyclableMaterial material : materials) {
            if (material.getCode() == code) {
                ...
                if (material.isRecyclable()) {
                    System.out.println("Положите его в контейнер с типом «"
                            + containers.get(material.getType()) + "».");
                } else {
    ...
    public static ArrayList<RecyclableMaterial> getMaterials() {
        ...
        materials.add(new RecyclableMaterial(1, MaterialType.PLASTIC, true, "полиэтилентерефталат"));
